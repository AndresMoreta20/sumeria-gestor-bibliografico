      ...
      <div v-else>
        ...
        >
          <CardBox
            ...
            :key="book.id"
            class="flex flex-col items-center h-full border rounded-lg p-4"
          >
            <img
              :src="book.images[0]?.src || 'default-cover.jpg'"
              alt="Book cover"
            ...
            <p v-if="book.virtual != true" class="text-blue-500 mb-2">
            ...
            <p class="text-gray-500 mb-4">
              {{
                book.attributes.find((attr) => attr.name === "Formato")
                  ?.options[0]
              }}
            </p>
            <div
            ...
            >
              <button
                ...
                class="bg-gray-300 text-black py-2 px-4 rounded w-full mr-2"
              >
                Ver
              </button>
              <button
...
const filteredBooks = computed(() => {
  ...
  filtered = filtered.filter(
    (book) => book.status === (showAvailable.value ? "publish" : "trash")
  );
...
const updateBookStatus = async (bookId, status) => {
  try {
    ...
    console.error(
      `Error al ${status === "publish" ? "activar" : "desactivar"} el libro:`,
...
const confirmUpdateStatus = () => {
  if (bookToUpdate.value) {
    updateBookStatus(
      bookToUpdate.value.id,
      bookToUpdate.value.status === "publish" ? "trash" : "publish"
    );
